 ...
 * -----------------------------------------------------------------------------*/
	...
	sendSz = InfoSz*50; // InfoSz*int(numparticles/(50*nprocs));
 ...
 * -----------------------------------------------------------------------------*/
	...
	generateSplines(W, splinep, den, u, v, w, p);		  		
...
void PrintStats(ptype *W[5], UBspline_3d_d *splinep[5])
{	
...
//					localwmean += wi; 
	...
	if (myrank_3d == 0) {			
		std::cout << scientific << t/t0 << "  " << TKE <<  "  " << Diss << "  " << SKEWu << "  " << SKEWv << "  " << SKEWw << "  " << KURTu << "  " << KURTv << "  " << KURTw << "  "  
        << Prms << "  " << Trms << "  " << "  " << Vrms  << std::endl;	
	}	
...
{	
	...
	MPI_Allreduce(&localTmean, &Tmean, 1, MPI_DOUBLE, MPI_SUM, COMM3D);
	MPI_Allreduce(&localVmean, &Vmean, 1, MPI_DOUBLE, MPI_SUM, COMM3D);
	MPI_Allreduce(&localumean, &umean, 1, MPI_DOUBLE, MPI_SUM, COMM3D);
	MPI_Allreduce(&localvmean, &vmean, 1, MPI_DOUBLE, MPI_SUM, COMM3D);
	MPI_Allreduce(&localwmean, &wmean, 1, MPI_DOUBLE, MPI_SUM, COMM3D);
			...
			For(k, nt_segz)
				{
				    ...
				    localTrms +=  pow((Tmean-Ti),2);
				    localVrms +=  pow((Vmean-1/deni),2);
				    localurms +=  pow((umean-ui),2);				
				    localvrms +=  pow((vmean-vi),2);				
				    localwrms +=  pow((wmean-wi),2);	
				}	
	
	MPI_Reduce(&localPrms, &Prms, 1, MPI_DOUBLE, MPI_SUM, 0, COMM3D);
	MPI_Reduce(&localTrms, &Trms, 1, MPI_DOUBLE, MPI_SUM, 0, COMM3D);
	MPI_Reduce(&localVrms, &Vrms, 1, MPI_DOUBLE, MPI_SUM, 0, COMM3D);
	MPI_Reduce(&localurms, &urms, 1, MPI_DOUBLE, MPI_SUM, 0, COMM3D);
	MPI_Reduce(&localvrms, &vrms, 1, MPI_DOUBLE, MPI_SUM, 0, COMM3D);
	MPI_Reduce(&localwrms, &wrms, 1, MPI_DOUBLE, MPI_SUM, 0, COMM3D);
	...
	TKE  = ( urms*urms + vrms*vrms + wrms*wrms )  / pow(c0*Mt, 2);
	Prms = Prms / (gam*p0*Mt*Mt);
	Trms = Trms / ((gam-1)*T0*Mt*Mt);
	Vrms = Vrms / (Mt*Mt/den0);
	...
	if (myrank_3d == 0) {			
		std::cout << scientific << t/t0 <<"     " << TKE << "     "  << Prms << "      " << Trms << "     " << "      " << Vrms << "     "  << dt << std::endl;	
	}	
...
void evolveParticles(Particle *particles, ptype **W, UBspline_3d_d *splinep[5], ptype *sendParticle[27], ptype *recvParticle[27]) 
{
	ptype vval;
	int sendCount[27] = {0};
	...
	while (q < count) {
		...
		particles[q].track(W, splinep, dt); 
...
void GetParticleStats(Particle *particles, UBspline_3d_d *splinep[5]) {
	totalCount = 0;
...
void generateSplines(ptype *W[5], UBspline_3d_d *splinep[5], ptype *den, ptype *u, ptype *v, ptype *w, ptype *p) 
{
	FOR(i, Nt_seg) {
		den[i] = W[0][i];
		u[i] = W[1][i]/den[i];
...
void locateParticles(Particle *particles, UBspline_3d_d *splinep[5]) {
...
void writeParticleData(Particle *particles) {
	...
	FOR(i, count) {
		...
		fprintf(pFile, "%.15e %.15e %.15e\n", particles[i].gradP[1], particles[i].gradP[0], particles[i].gradP[2]);	
	}
	fclose(pFile);
...
void writeVelGradTensor(UBspline_3d_d *splinep[5]) {
	FILE * pFile;
	char Field[50];
