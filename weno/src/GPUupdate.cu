 ...
 */
__global__ void derivsX_Kernel(ptype *d_W, ptype *d_DWxl, ptype *d_DWyl, ptype *d_DWzl, ptype *d_DWxr, ptype *d_DWyr, ptype *d_DWzr);

__global__ void derivsY_Kernel(ptype *d_W, ptype *d_DWxl, ptype *d_DWyl, ptype *d_DWzl, ptype *d_DWxr, ptype *d_DWyr, ptype *d_DWzr);

__global__ void derivsZ_Kernel(ptype *d_W, ptype *d_DWxl, ptype *d_DWyl, ptype *d_DWzl, ptype *d_DWxr, ptype *d_DWyr, ptype *d_DWzr);

__global__ void flip_Kernel(ptype *d_DW1xl, ptype *d_DW2xl, ptype *d_DW1yl, ptype *d_DW2yl, ptype *d_DW1zl, ptype *d_DW2zl,
                            ptype *d_DW1xr, ptype *d_DW2xr, ptype *d_DW1yr, ptype *d_DW2yr, ptype *d_DW1zr, ptype *d_DW2zr);
...
__device__ void d_ApplyWENO(ptype WLLL, ptype WLL, ptype WL, ptype WR,
		                  ptype WRR,  ptype WRRR, ptype &Wl, ptype &Wr);
 ...
 * ----------------------------------------------------*/
	...
	derivsX_Kernel<<<blkst, Bszs>>>(dev->d_W0, dev->d_DW0xl, dev->d_DW0yl, dev->d_DW0zl, dev->d_DW0xr, dev->d_DW0yr, dev->d_DW0zr);
	derivsX_Kernel<<<blkst, Bszs>>>(dev->d_W1, dev->d_DW1xl, dev->d_DW1yl, dev->d_DW1zl, dev->d_DW1xr, dev->d_DW1yr, dev->d_DW1zr);
	derivsX_Kernel<<<blkst, Bszs>>>(dev->d_W2, dev->d_DW2xl, dev->d_DW2yl, dev->d_DW2zl, dev->d_DW2xr, dev->d_DW2yr, dev->d_DW2zr);
	derivsX_Kernel<<<blkst, Bszs>>>(dev->d_W3, dev->d_DW3xl, dev->d_DW3yl, dev->d_DW3zl, dev->d_DW3xr, dev->d_DW3yr, dev->d_DW3zr);
	derivsX_Kernel<<<blkst, Bszs>>>(dev->d_W4, dev->d_DW4xl, dev->d_DW4yl, dev->d_DW4zl, dev->d_DW4xr, dev->d_DW4yr, dev->d_DW4zr);
	...
	derivsY_Kernel<<<blkst, Bszs>>>(dev->d_W0, dev->d_DW0xl, dev->d_DW0yl, dev->d_DW0zl, dev->d_DW0xr, dev->d_DW0yr, dev->d_DW0zr);
	derivsY_Kernel<<<blkst, Bszs>>>(dev->d_W1, dev->d_DW1xl, dev->d_DW1yl, dev->d_DW1zl, dev->d_DW1xr, dev->d_DW1yr, dev->d_DW1zr);
	derivsY_Kernel<<<blkst, Bszs>>>(dev->d_W2, dev->d_DW2xl, dev->d_DW2yl, dev->d_DW2zl, dev->d_DW2xr, dev->d_DW2yr, dev->d_DW2zr);
	derivsY_Kernel<<<blkst, Bszs>>>(dev->d_W3, dev->d_DW3xl, dev->d_DW3yl, dev->d_DW3zl, dev->d_DW3xr, dev->d_DW3yr, dev->d_DW3zr);
	derivsY_Kernel<<<blkst, Bszs>>>(dev->d_W4, dev->d_DW4xl, dev->d_DW4yl, dev->d_DW4zl, dev->d_DW4xr, dev->d_DW4yr, dev->d_DW4zr);
		
	cudaDeviceSynchronize();
		
	flip_Kernel<<<blkz, Bsz3D>>>(dev->d_DW1xl, dev->d_DW2xl, dev->d_DW1yl, dev->d_DW2yl, dev->d_DW1zl, dev->d_DW2zl, 
												 dev->d_DW1xr, dev->d_DW2xr, dev->d_DW1yr, dev->d_DW2yr, dev->d_DW1zr, dev->d_DW2zr);
	(cudaDeviceSynchronize());
	...
	derivsZ_Kernel<<<blkst, Bszs>>>(dev->d_W0, dev->d_DW0xl, dev->d_DW0yl, dev->d_DW0zl, dev->d_DW0xr, dev->d_DW0yr, dev->d_DW0zr);
	derivsZ_Kernel<<<blkst, Bszs>>>(dev->d_W1, dev->d_DW1xl, dev->d_DW1yl, dev->d_DW1zl, dev->d_DW1xr, dev->d_DW1yr, dev->d_DW1zr);
	derivsZ_Kernel<<<blkst, Bszs>>>(dev->d_W2, dev->d_DW2xl, dev->d_DW2yl, dev->d_DW2zl, dev->d_DW2xr, dev->d_DW2yr, dev->d_DW2zr);
	derivsZ_Kernel<<<blkst, Bszs>>>(dev->d_W3, dev->d_DW3xl, dev->d_DW3yl, dev->d_DW3zl, dev->d_DW3xr, dev->d_DW3yr, dev->d_DW3zr);
	derivsZ_Kernel<<<blkst, Bszs>>>(dev->d_W4, dev->d_DW4xl, dev->d_DW4yl, dev->d_DW4zl, dev->d_DW4xr, dev->d_DW4yr, dev->d_DW4zr);
		
	cudaDeviceSynchronize();
		
	flip_Kernel<<<blkz, Bsz3D>>>(dev->d_DW1xl, dev->d_DW3xl, dev->d_DW1yl, dev->d_DW3yl, dev->d_DW1zl, dev->d_DW3zl, 
								 dev->d_DW1xr, dev->d_DW3xr, dev->d_DW1yr, dev->d_DW3yr, dev->d_DW1zr, dev->d_DW3zr);
...
__global__ void derivsX_Kernel(ptype *d_W, ptype *d_DWxl, ptype *d_DWyl, ptype *d_DWzl, ptype *d_DWxr, ptype *d_DWyr, ptype *d_DWzr)
{
	...
	ptype WLLL, WLL, WL, WR, WRR, WRRR;
	ptype WLLLN, WLLN, WLN, WRN, WRRN, WRRRN;
	ptype WLLLS, WLLS, WLS, WRS, WRRS, WRRRS;
	ptype WLLLF, WLLF, WLF, WRF, WRRF, WRRRF;
	ptype WLLLB, WLLB, WLB, WRB, WRRB, WRRRB;
	ptype Wl, Wr, WlN, WrN, WlS, WrS, WlF, WrF, WlB, WrB;
			
	if(ix < d_nt_segx-3 && iy < d_nt_segy-3 && iz < d_nt_segz-3 && iy > 2 && iz > 2 && ix > 1)
	{		
		WLLL = d_W[Id(iy,ix-2,iz)];
		WLL  = d_W[Id(iy,ix-1,iz)];
		WL   = d_W[Id(iy,ix,iz)];
		WR   = d_W[Id(iy,ix+1,iz)];
		WRR  = d_W[Id(iy,ix+2,iz)];
		WRRR = d_W[Id(iy,ix+3,iz)];
		
		WLLLN = d_W[Id(iy+1,ix-2,iz)];
		WLLN  = d_W[Id(iy+1,ix-1,iz)];
		WLN   = d_W[Id(iy+1,ix,iz)];
		WRN   = d_W[Id(iy+1,ix+1,iz)];
		WRRN  = d_W[Id(iy+1,ix+2,iz)];
		WRRRN = d_W[Id(iy+1,ix+3,iz)];
		
		WLLLS = d_W[Id(iy-1,ix-2,iz)];
		WLLS  = d_W[Id(iy-1,ix-1,iz)];
		WLS   = d_W[Id(iy-1,ix,iz)];
		WRS   = d_W[Id(iy-1,ix+1,iz)];
		WRRS  = d_W[Id(iy-1,ix+2,iz)];
		WRRRS = d_W[Id(iy-1,ix+3,iz)];

		WLLLF = d_W[Id(iy,ix-2,iz+1)];
		WLLF  = d_W[Id(iy,ix-1,iz+1)];
		WLF   = d_W[Id(iy,ix,iz+1)];
		WRF   = d_W[Id(iy,ix+1,iz+1)];
		WRRF  = d_W[Id(iy,ix+2,iz+1)];
		WRRRF = d_W[Id(iy,ix+3,iz+1)];

		WLLLB = d_W[Id(iy,ix-2,iz-1)];
		WLLB  = d_W[Id(iy,ix-1,iz-1)];
		WLB   = d_W[Id(iy,ix,iz-1)];
		WRB   = d_W[Id(iy,ix+1,iz-1)];
		WRRB  = d_W[Id(iy,ix+2,iz-1)];
		WRRRB = d_W[Id(iy,ix+3,iz-1)];
		...
		d_DWxl[Id(iy,ix,iz)] = 2*(Wl - WL) / d_dx; 		
		d_DWxr[Id(iy,ix,iz)] = 2*(WR - Wr) / d_dx;
		
		d_DWyl[Id(iy,ix,iz)] = 0.5*(WlN - WlS) / d_dx;
		d_DWyr[Id(iy,ix,iz)] = 0.5*(WrN - WrS) / d_dx;
		d_DWzl[Id(iy,ix,iz)] = 0.5*(WlF - WlB) / d_dx;
		d_DWzr[Id(iy,ix,iz)] = 0.5*(WrF - WrB) / d_dx;
	}	
}
/**-----------------------------------------------------------------------------------------*
...
__global__ void derivsY_Kernel(ptype *d_W, ptype *d_DWxl, ptype *d_DWyl, ptype *d_DWzl, ptype *d_DWxr, ptype *d_DWyr, ptype *d_DWzr)
{
	...
	{		
		WLLL = d_W[Id(iy-2,ix,iz)];
		WLL  = d_W[Id(iy-1,ix,iz)];
		WL   = d_W[Id(iy,ix,iz)];
		WR   = d_W[Id(iy+1,ix,iz)];
		WRR  = d_W[Id(iy+2,ix,iz)];
		WRRR = d_W[Id(iy+3,ix,iz)];
		
		WLLLN = d_W[Id(iy-2,ix-1,iz)];
		WLLN  = d_W[Id(iy-1,ix-1,iz)];
		WLN   = d_W[Id(iy,ix-1,iz)];
		WRN   = d_W[Id(iy+1,ix-1,iz)];
		WRRN  = d_W[Id(iy+2,ix-1,iz)];
		WRRRN = d_W[Id(iy+3,ix-1,iz)];
		
		WLLLS = d_W[Id(iy-2,ix+1,iz)];
		WLLS  = d_W[Id(iy-1,ix+1,iz)];
		WLS   = d_W[Id(iy,ix+1,iz)];
		WRS   = d_W[Id(iy+1,ix+1,iz)];
		WRRS  = d_W[Id(iy+2,ix+1,iz)];
		WRRRS = d_W[Id(iy+3,ix+1,iz)];

		WLLLF = d_W[Id(iy-2,ix,iz+1)];
		WLLF  = d_W[Id(iy-1,ix,iz+1)];
		WLF   = d_W[Id(iy,ix,iz+1)];
		WRF   = d_W[Id(iy+1,ix,iz+1)];
		WRRF  = d_W[Id(iy+2,ix,iz+1)];
		WRRRF = d_W[Id(iy+3,ix,iz+1)];

		WLLLB = d_W[Id(iy-2,ix,iz-1)];
		WLLB  = d_W[Id(iy-1,ix,iz-1)];
		WLB   = d_W[Id(iy,ix,iz-1)];
		WRB   = d_W[Id(iy+1,ix,iz-1)];
		WRRB  = d_W[Id(iy+2,ix,iz-1)];
		WRRRB = d_W[Id(iy+3,ix,iz-1)];
...
__global__ void derivsZ_Kernel(ptype *d_W, ptype *d_DWxl, ptype *d_DWyl, ptype *d_DWzl, ptype *d_DWxr, ptype *d_DWyr, ptype *d_DWzr)
{
	...
	{		
		WLLL = d_W[Id(iy,ix,iz-2)];
		WLL  = d_W[Id(iy,ix,iz-1)];
		WL   = d_W[Id(iy,ix,iz)];
		WR   = d_W[Id(iy,ix,iz+1)];
		WRR  = d_W[Id(iy,ix,iz+2)];
		WRRR = d_W[Id(iy,ix,iz+3)];
		
		WLLLN = d_W[Id(iy,ix+1,iz-2)];
		WLLN  = d_W[Id(iy,ix+1,iz-1)];
		WLN   = d_W[Id(iy,ix+1,iz)];
		WRN   = d_W[Id(iy,ix+1,iz+1)];
		WRRN  = d_W[Id(iy,ix+1,iz+2)];
		WRRRN = d_W[Id(iy,ix+1,iz+3)];
		
		WLLLS = d_W[Id(iy,ix-1,iz-2)];
		WLLS  = d_W[Id(iy,ix-1,iz-1)];
		WLS   = d_W[Id(iy,ix-1,iz)];
		WRS   = d_W[Id(iy,ix-1,iz+1)];
		WRRS  = d_W[Id(iy,ix-1,iz+2)];
		WRRRS = d_W[Id(iy,ix-1,iz+3)];

		WLLLF = d_W[Id(iy+1,ix,iz-2)];
		WLLF  = d_W[Id(iy+1,ix,iz-1)];
		WLF   = d_W[Id(iy+1,ix,iz)];
		WRF   = d_W[Id(iy+1,ix,iz+1)];
		WRRF  = d_W[Id(iy+1,ix,iz+2)];
		WRRRF = d_W[Id(iy+1,ix,iz+3)];

		WLLLB = d_W[Id(iy-1,ix,iz-2)];
		WLLB  = d_W[Id(iy-1,ix,iz-1)];
		WLB   = d_W[Id(iy-1,ix,iz)];
		WRB   = d_W[Id(iy-1,ix,iz+1)];
		WRRB  = d_W[Id(iy-1,ix,iz+2)];
		WRRRB = d_W[Id(iy-1,ix,iz+3)];
 ...
 * ----------------------------------------------------------------------------------------------------------------------------------*/	
	...
	if (bound_check) {
		...
		if (TAG == 1) {
			...
			d_ApplyWENO(d_W0[Id(iy,ix-2,iz)], d_W0[Id(iy,ix-1,iz)],  d_W0[Id(iy,ix,iz)], d_W0[Id(iy,ix+1,iz)], d_W0[Id(iy,ix+2,iz)], d_W0[Id(iy,ix+3,iz)], Wl[0], Wr[0]);
			d_ApplyWENO(d_W1[Id(iy,ix-2,iz)], d_W1[Id(iy,ix-1,iz)],  d_W1[Id(iy,ix,iz)], d_W1[Id(iy,ix+1,iz)], d_W1[Id(iy,ix+2,iz)], d_W1[Id(iy,ix+3,iz)], Wl[1], Wr[1]);
			d_ApplyWENO(d_W2[Id(iy,ix-2,iz)], d_W2[Id(iy,ix-1,iz)],  d_W2[Id(iy,ix,iz)], d_W2[Id(iy,ix+1,iz)], d_W2[Id(iy,ix+2,iz)], d_W2[Id(iy,ix+3,iz)], Wl[2], Wr[2]);
			d_ApplyWENO(d_W3[Id(iy,ix-2,iz)], d_W3[Id(iy,ix-1,iz)],  d_W3[Id(iy,ix,iz)], d_W3[Id(iy,ix+1,iz)], d_W3[Id(iy,ix+2,iz)], d_W3[Id(iy,ix+3,iz)], Wl[3], Wr[3]);
			d_ApplyWENO(d_W4[Id(iy,ix-2,iz)], d_W4[Id(iy,ix-1,iz)],  d_W4[Id(iy,ix,iz)], d_W4[Id(iy,ix+1,iz)], d_W4[Id(iy,ix+2,iz)], d_W4[Id(iy,ix+3,iz)], Wl[4], Wr[4]);	
		...
		if (TAG == 2) {
			...
			d_ApplyWENO(d_W0[Id(iy-2,ix,iz)], d_W0[Id(iy-1,ix,iz)],  d_W0[Id(iy,ix,iz)], d_W0[Id(iy+1,ix,iz)], d_W0[Id(iy+2,ix,iz)], d_W0[Id(iy+3,ix,iz)], Wl[0], Wr[0]);
			d_ApplyWENO(d_W1[Id(iy-2,ix,iz)], d_W1[Id(iy-1,ix,iz)],  d_W1[Id(iy,ix,iz)], d_W1[Id(iy+1,ix,iz)], d_W1[Id(iy+2,ix,iz)], d_W1[Id(iy+3,ix,iz)], Wl[2], Wr[2]);
			d_ApplyWENO(d_W2[Id(iy-2,ix,iz)], d_W2[Id(iy-1,ix,iz)],  d_W2[Id(iy,ix,iz)], d_W2[Id(iy+1,ix,iz)], d_W2[Id(iy+2,ix,iz)], d_W2[Id(iy+3,ix,iz)], Wl[1], Wr[1]);
			d_ApplyWENO(d_W3[Id(iy-2,ix,iz)], d_W3[Id(iy-1,ix,iz)],  d_W3[Id(iy,ix,iz)], d_W3[Id(iy+1,ix,iz)], d_W3[Id(iy+2,ix,iz)], d_W3[Id(iy+3,ix,iz)], Wl[3], Wr[3]);
			d_ApplyWENO(d_W4[Id(iy-2,ix,iz)], d_W4[Id(iy-1,ix,iz)],  d_W4[Id(iy,ix,iz)], d_W4[Id(iy+1,ix,iz)], d_W4[Id(iy+2,ix,iz)], d_W4[Id(iy+3,ix,iz)], Wl[4], Wr[4]);				
		}
		// Z-Flux
		if (TAG == 3) {
			WL[0] = d_W0[Id(iy,ix,iz)];
			...
			d_ApplyWENO(d_W0[Id(iy,ix,iz-2)], d_W0[Id(iy,ix,iz-1)],  d_W0[Id(iy,ix,iz)], d_W0[Id(iy,ix,iz+1)], d_W0[Id(iy,ix,iz+2)], d_W0[Id(iy,ix,iz+3)], Wl[0], Wr[0]);
			d_ApplyWENO(d_W1[Id(iy,ix,iz-2)], d_W1[Id(iy,ix,iz-1)],  d_W1[Id(iy,ix,iz)], d_W1[Id(iy,ix,iz+1)], d_W1[Id(iy,ix,iz+2)], d_W1[Id(iy,ix,iz+3)], Wl[3], Wr[3]);
			d_ApplyWENO(d_W2[Id(iy,ix,iz-2)], d_W2[Id(iy,ix,iz-1)],  d_W2[Id(iy,ix,iz)], d_W2[Id(iy,ix,iz+1)], d_W2[Id(iy,ix,iz+2)], d_W2[Id(iy,ix,iz+3)], Wl[2], Wr[2]);
			d_ApplyWENO(d_W3[Id(iy,ix,iz-2)], d_W3[Id(iy,ix,iz-1)],  d_W3[Id(iy,ix,iz)], d_W3[Id(iy,ix,iz+1)], d_W3[Id(iy,ix,iz+2)], d_W3[Id(iy,ix,iz+3)], Wl[1], Wr[1]);
			d_ApplyWENO(d_W4[Id(iy,ix,iz-2)], d_W4[Id(iy,ix,iz-1)],  d_W4[Id(iy,ix,iz)], d_W4[Id(iy,ix,iz+1)], d_W4[Id(iy,ix,iz+2)], d_W4[Id(iy,ix,iz+3)], Wl[4], Wr[4]);	
		}
...
__device__ void d_slopesolver(ptype b[5], ptype U[3], ptype lam, ptype a[5])
{
	...
	a[4] = (R5-2*U[0]*R2-2*U[1]*R3-2*U[2]*R4)*(4*lam*lam)/(d_K+3);
 ...
 *  WENO - SYMOO
 * ---------------------------------------------------------------------------------------------------------------------------------------------------*/
__device__ void d_ApplyWENO(ptype WLLL, ptype WLL, ptype WL, ptype WR, ptype WRR,  ptype WRRR, ptype &Wl, ptype &Wr)
{	
//	ptype TL, TLL, TLLL, TR, TRR, TRRR;
	...
	gam0 = 1.0/20.0, gam1 = 9.0/20.0, gam2 = 9.0/20.0, gam3 = 1.0/20.0;
		...
		P0p =     WLLL/3  - 7*WLL/6  + 11*WL/6;
		P0n =     WRRR/3  - 7*WRR/6  + 11*WR/6;
		P1p =    -WLL/6   + 5*WL/6   + WR/3;
		P1n =    -WRR/6   + 5*WR/6   + WL/3;	
		P2p =     WL/3    + 5*WR/6   - WRR/6;
		P2n =     WR/3    + 5*WL/6   - WLL/6;
		P3p =  11*WR/6    - 7*WRR/6  + WRRR/3;
		P3n =  11*WL/6    - 7*WLL/6  + WLLL/3;
			
		IS0p = 13*pow((WLLL - 2*WLL + WL),2)/12    +  pow(( WLLL - 4*WLL + 3*WL),2)/4;
		IS0n = 13*pow((WRRR - 2*WRR + WR),2)/12    +  pow(( WRRR - 4*WRR + 3*WR),2)/4;
		IS1p = 13*pow((WLL  - 2*WL  + WR),2)/12    +  pow(( -WLL + WR),2)/4;
		IS1n = 13*pow((WRR  - 2*WR  + WL),2)/12    +  pow(( -WRR + WL),2)/4;
		IS2p = 13*pow((WL   - 2*WR  + WRR),2)/12   +  pow((-3*WL + 4*WR - WRR),2)/4;
		IS2n = 13*pow((WR   - 2*WL  + WLL),2)/12   +  pow((-3*WR + 4*WL - WLL),2)/4;
		IS3p = 13*pow((WR   - 2*WRR + WRRR),2)/12  +  pow((-5*WR + 8*WRR - 3*WRRR),2)/4;
		IS3n = 13*pow((WL   - 2*WLL + WLLL),2)/12  +  pow((-5*WL + 8*WLL - 3*WLLL),2)/4;
		...
		Wl = (OMEGA0p*P0p + OMEGA1p*P1p + OMEGA2p*P2p + OMEGA3p*P3p); 
		Wr = (OMEGA0n*P0n + OMEGA1n*P1n + OMEGA2n*P2n + OMEGA3n*P3n); 
}
