...
void Particle::track(ptype *W[5], UBspline_3d_d *splinep[5], ptype dt) {
	updatePosition(W, splinep, dt);
	identifyBlockId();
}

void Particle::updatePosition(ptype *W[5], UBspline_3d_d *splinep[5], ptype dt) {

/**	RK-2 (HEUN)
 */ 
/*	ptype U2[3];
	eval_UBspline_3d_d (splinep[1], pos[0] + 0.5*U[1]*dt, pos[1] + 0.5*U[0]*dt, pos[2] + 0.5*U[2]*dt, &U2[0]);
	eval_UBspline_3d_d (splinep[2], pos[0] + 0.5*U[1]*dt, pos[1] + 0.5*U[0]*dt, pos[2] + 0.5*U[2]*dt, &U2[1]);
	eval_UBspline_3d_d (splinep[3], pos[0] + 0.5*U[1]*dt, pos[1] + 0.5*U[0]*dt, pos[2] + 0.5*U[2]*dt, &U2[2]);
*/	
	pos[0] +=  U[1]*dt; //0.5*(U[1] + U2[1])*dt;
	pos[1] +=  U[0]*dt; //0.5*(U[0] + U2[0])*dt;
	pos[2] +=  U[2]*dt; //0.5*(U[2] + U2[2])*dt;
...
void Particle::getParticleProp(UBspline_3d_d *splinep[5]) {
	...
	eval_UBspline_3d_d_vg (splinep[4], pos[0], pos[1], pos[2], &P   , gradP);	
}
